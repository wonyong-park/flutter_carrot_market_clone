...
class ContentsRepository extends LocalStorageRepository {
  ...
  Future<dynamic> loadContentFromLocation(String location) async {
    //Api 통신 한다고 가정
    await Future.delayed(Duration(seconds: 1)); //3초 딜레이
    return data[location];
  }

  Future<List<dynamic>?> loadFavoriteContents() async{
    String? jsonString = await getStoreValue(MY_FAVORITE_STORE_KEY);
    if (jsonString != null) {
  ...
  addMyFavoriteContent(Map<String, String> content) async {
    ...
    await updateFavoriteContent(favoriteContentsList);
  }
  ...
  updateFavoriteContent(List<dynamic>? favoriteContentsList) async {
    await storeValue(MY_FAVORITE_STORE_KEY, jsonEncode(favoriteContentsList));
  }

  Future<bool> isMyFavoriteContents(String cid) async {
    bool isMyFavorContent = false;
    ...
    if (jsonList != null) {
      ...
      for (dynamic data in jsonList) {
        if(data["cid"] == cid) {
          isMyFavorContent = true;
          break;
        }
      }

      return isMyFavorContent;
    } else {
      return false;
    }
  }
